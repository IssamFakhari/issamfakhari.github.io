...
cred = 'b0ad2c8cf5daf4e7e285034e07aec2e0b4901ff5'
set_default_credentials(cred)

df = gpd.read_file('clean_data.csv')
...
gdf = gpd.GeoDataFrame(df,
                      geometry=gpd.points_from_xy(x=df.longitude, 
                                                  y=df.latitude))
gdf.head()
#gdf.rename(columns={"geometry":"the_geom"}, inplace = True ) 


gdf['price'] = pd.to_numeric(df['price'], errors='coerce')
...
propuesta_gdf = Geocoding().geocode()
```


```python
from cartoframes.viz import Map, Layer, size_continuous_style, size_continuous_legend, color_bins_style

#size_continuous_style('price', size_range=[8,70]
#Map([
Layer(gdf,
     color_bins_style('price'))
          #])
